...
add_action( 'wp_enqueue_scripts', 'wds_kamu_scripts' );
 ...
 * @author WebDevStudios
 */
function wds_kamu_preload_scripts() {
	$asset_file_path = dirname( __DIR__ ) . '/build/index.asset.php';
...
add_action( 'wp_head', 'wds_kamu_preload_scripts', 1 );
 ...
 */
function wds_kamu_preload_assets() {
	?>
	<?php if ( wds_kamu_get_custom_logo_url() ) : ?>
		<link rel="preload" href="<?php echo esc_url( wds_kamu_get_custom_logo_url() ); ?>" as="image">
	<?php endif; ?>
	<?php
}
add_action( 'wp_head', 'wds_kamu_preload_assets', 1 );
