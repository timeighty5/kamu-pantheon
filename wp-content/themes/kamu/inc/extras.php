 ...
 */
function wds_kamu_categorized_blog() {
	$category_count = get_transient( 'wds_kamu_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'wds_kamu_categories', $category_count );
	}
 ...
 */
function wds_kamu_get_attachment_id_from_url( $attachment_url = '' ) {
	global $wpdb;
 ...
 */
function wds_kamu_copyright_year( $atts ) {
	// Setup defaults.
	$args = shortcode_atts(
		[
			'starting_year' => '',
...
add_shortcode( 'wds_kamu_copyright_year', 'wds_kamu_copyright_year', 15 );
 ...
 */
function wds_kamu_get_custom_logo_url() {
